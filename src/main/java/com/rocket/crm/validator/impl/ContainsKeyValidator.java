...
public class ContainsKeyValidator implements ConstraintValidator<ContainsKeys, Map<String, ? extends Object>> {
	...
	public boolean isValid(Map<String, ? extends Object> map, ConstraintValidatorContext context) {
		...
		for (String key : keys) {
			if (!this.validate(map, key)) {
				context.disableDefaultConstraintViolation();
				context.buildConstraintViolationWithTemplate(String.format("%s key not exist.", key))
